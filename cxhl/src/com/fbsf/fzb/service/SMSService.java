...
public class SMSService extends Service {
	...
	{
		int rowNum =0;
		int id =getTableSequence("rent_sms", "id", 1);
		row.put("id", id);
		row.put("send_time", DateUtil.getCurrentDateTime());
		rowNum =insert("rent_sms", row);
		return rowNum;
	}
	...
	public boolean findByCodeAndTelphone(String code,String telphone)
	{
		...
		sql ="select count(*) from rent_sms where sms_code='"+code+"' and to_account='"+telphone+"' and status='1' ";
	...
	{
		boolean existed =true;
		sql ="select count(*) from rent_sms where sms_code='"+code+"' and to_account='"+telphone+"' and status='1'  and type='"+type+"'";
		int num =Integer.parseInt(queryField(sql));
		if(num == 0)
		{
			existed =false;
	...
	{
		int num =0;
		sql ="select count(*) from rent_sms where to_account='"+telephone+"' and status='1' ";
	...
	public void setCodeTimeOut()
	{
		...
		String sql ="select * from rent_sms where status='1' ";
		...
		if(ds !=null && ds.size()>0)
		{
			...
			for(int i=0;i<ds.size();i++)
			{
				...
				if(minus >=5)
				{
					...
					String t_sql ="update rent_sms set status='2' where id='"+id+"' ";
					update(t_sql);
				}
			}
		}
	...
	{
		Row row =null;
		String sql ="select * from rent_sms where to_account='"+telephone+"' and status='1' order by send_time desc limit 0,1";
		row =queryRow(sql);
		return row;
	}
	...
	{
		String sql ="update rent_sms set status='2' where to_account='"+telephone+"' and status='1'";
		update(sql);
	}
