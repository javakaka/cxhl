...
public class VersionService extends Service {
	...
	{
		int rowNum =0;
		int id =getTableSequence("rent_app_version", "id", 1);
		row.put("id", id);
		row.put("create_time", DateUtil.getCurrentDateTime());
		rowNum =insert("rent_app_version", row);
		return rowNum;
	}
	...
	{
		Row row =null;
		sql ="select * from rent_app_version where id='"+id+"'";
		row =queryRow(sql);
		return row;
	}
	...
	{
		Row row =null;
		sql ="select * from rent_app_version where app_id='"+app+"' and device='"+device+"'";
		row =queryRow(sql);
		return row;
	}
	...
	public int update(Row row)
	{
		...
		update("rent_app_version", row, " id='"+id+"'");
		return rowNum;
	}
	...
	public DataSet list()
	{
		...
		sql ="select id,name,picture,width,height from rent_app_version where status=1 and (start_time<='"+now_time+"' and end_time>='"+now_time+"')";
		ds =queryDataSet(sql);
		return ds;
	...
	public void delete(Long... ids) {
		...
		if (ids != null) {
			...
			sql = "delete from rent_app_version where id in(" + id + ")";
			update(sql);
		}
	}
	...
	{
		//update db record
		String sql =" update rent_app_version set is_current_version='2' where app_id='"+app_id+"' and device='"+device+"'";
		update(sql);
		sql ="update rent_app_version set is_current_version='1' where id='"+version_id+"'";
		update(sql);
