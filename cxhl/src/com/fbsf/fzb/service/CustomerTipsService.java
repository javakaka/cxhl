 ...
 * 类说明:在线客服用户反馈业务处理类 
 */
@Component("fzbCustomerTipsService")
public class CustomerTipsService extends Service {
	...
	{
		int rowNum =0;
		int id =getTableSequence("rent_customer_tips", "id", 1);
		row.put("id", id);
		row.put("create_time", DateUtil.getCurrentDateTime());
		rowNum =insert("rent_customer_tips", row);
		return rowNum;
	}
	...
	{
		Row row =null;
		sql ="select * from rent_customer_tips where id='"+id+"'";
		row =queryRow(sql);
		return row;
	}
	...
	public int update(Row row)
	{
		...
		update("rent_customer_tips", row, " id='"+id+"'");
		return rowNum;
	}
	...
	public DataSet list(int page,int page_size,String user_id)
	{
		...
		sql =" select * from rent_customer_tips where user_id='"+user_id+"' "
	...
	{
		int num =0;
		String sql ="select count(*) from rent_customer_tips where to_id='"+user_id+"'";
	...
	{
		int num =0;
		String sql ="select count(*) from rent_customer_tips where from_id='"+user_id+"'";
		num =Integer.parseInt(queryField(sql));
		return num;
	}
	...
	{
		int num =0;
		String sql ="select count(*) from rent_customer_tips where to_id='"+user_id+"' and read_status='2' ";
		num =Integer.parseInt(queryField(sql));
		return num;
	}
	...
	{
		int num =0;
		String sql ="select count(*) from rent_customer_tips where to_id='"+user_id+"'  and read_status='1' ";
		num =Integer.parseInt(queryField(sql));
		return num;
	}
	...
	public Page queryPage(String status,Pageable pageable) {
		Page page = null;
		sql ="select * from rent_customer_tips where 1=1 "; 
		...
		sql += restrictions;
		sql += orders;
		String countSql ="select count(*) from rent_customer_tips where 1=1 "; 
		if(! StringUtils.isEmptyOrNull(status))
		{
	...
	public void delete(Long... ids) {
		...
		if (ids != null) {
			...
			sql = "delete from rent_customer_tips where id in(" + id + ")";
			update(sql);
		}
	}
